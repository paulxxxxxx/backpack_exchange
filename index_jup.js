...
const init = async (client) => {
    ...
    try {
        ...
        console.log(getNowFormatDate(), "等待....随机....N秒...");
        await delay(1000,3000); // 在每次操作之前等待3秒
        ...
        await delay(1000,2000); // 在重新尝试之前等待1秒
    }
}
...
const sellfun = async (client) => {
    ...
    console.log(getNowFormatDate(), "正在获取JUP_USDC的市场当前价格中...");
    // 获取SOL/USDC的当前市场价格
    let { lastPrice: lastPriceask } = await client.Ticker({ symbol: "JUP_USDC" });
    // 调用API获取SOL/USDC的最后成交价
    console.log(getNowFormatDate(), "JUP_USDC的市场当前价格:", lastPriceask);
    // 打印当前市场价格

    let quantitys = ((userbalance2.JUP.available / 2) - 2).toFixed(2).toString();//------------>>>>>>>>>>>>>需要测试
    // 计算卖出数量，这里示例中卖出可用SOL的一半减去固定数量（0.02个），并将结果格式化为字符串
    console.log(getNowFormatDate(), `正在卖出中... 卖${quantitys}个JUP`);
    // 记录卖出操作
...
const buyfun = async (client) => {
    ...
    // 调用API获取SOL/USDC的最后成交价
    console.log(getNowFormatDate(), "JUP_USDC的市场当前价格:", lastPrice);
    // 打印当前市场价格

    console.log(getNowFormatDate(), `正在买入中... 花${(userbalance.USDC.available - 2).toFixed(2).toString()}个USDC买 JUP`);
